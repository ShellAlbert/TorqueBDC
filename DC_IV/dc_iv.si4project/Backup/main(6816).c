...
int main(void)
{
	...
	while (1) {
		...
		if (HAL_GetTick() % 50 == 0 && flag == 0) {
			...
			//current<80mA, fang xian -> shou xian.
			//current>140mA, shouxian -> fang xian.
			if (iCurrent < 200) {
				set_motor_direction(MOTOR_FWD); 	//shouxian.
				printf("<<<<<<<<<<-------shouxian\r\n");
			}
			else if (iCurrent >= 200 && iCurrent <= 300) {
				printf("------>>>>>>\r\n");
				set_motor_direction(MOTOR_REV); 	//fangxian.
				HAL_Delay(1000);//skip reserve big current.
				
				//delay 500ms.
				HAL_Delay(3000);
				printf("<<<<<<------\r\n");
				set_motor_direction(MOTOR_FWD); 	//shouxian.
				//skip reserve big current.
				HAL_Delay(1000);
			}
			else if (iCurrent > 500) {
				//something happened,stop immediately.
				printf("!!!!!!!!!!!!!!\r\n");
				set_motor_disable();
			}
		}
